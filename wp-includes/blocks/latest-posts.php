...
function render_block_core_latest_posts($attributes)
{
	...
	foreach ($recent_posts as $post) {
		...
		// Tách ngày, tháng, năm
		$day   = get_the_date('d', $post); // Lấy ngày
		$month = get_the_date('m', $post); // Lấy tháng
		...
		$list_items_markup .= sprintf('<span class="month">%s</span>', esc_html($month));
		$list_items_markup .= '</div>';
		$list_items_markup .= sprintf('<span class="year">%s</span>', esc_html($year));
		$list_items_markup .= '</div>';
		$list_items_markup .= sprintf(
...
<style>
	...
	.wp-block-latest-posts__post-date .day-month {
		...
		font-size: 20px;
		/* Kích thước lớn hơn cho ngày và tháng */
		font-weight: bold;
		line-height: 1;
		text-align: center;
	...
	.wp-block-latest-posts__post-date .day {
		...
		margin-bottom: -4px;
		/* Điều chỉnh khoảng cách để tạo hiệu ứng phân số */
	...
	.wp-block-latest-posts__post-date .year {
		...
		/* Màu chữ cho năm */
		margin-left: 5px;
		/* Khoảng cách giữa phân số và năm */
		font-weight: bold;
		color: white !important;
		font-weight: 100 !important;
		width: 20px;
